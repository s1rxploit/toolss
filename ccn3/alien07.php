...
$keredit=$credit-4;
if ($b == 1) {
mysql_query("UPDATE `user` SET `credits`=credits-5 WHERE email='$email'");
echo '{"error":0,"msg":"<font color=green><b>Live</b></font> | '.$ccnum1.' [CRE:'.$keredit.'] [GATE:03] ./JH3 Ceker"}';
break;
} if ($b == 2) {
echo '{"error":2,"msg":"<font color=red><b>Die</b></font> | '.$ccnum1.' '.$cre.' [GATE:03] ./JH3 Ceker"}';
break;
} if ($b == 3) {
echo '{"error":-1,"msg":"<font color=purple><b>Unknown</b></font> | '.$format.' | '.$cre.' [GATE:03] ./JH3 Ceker"}';
break;
}
if ($b == 4) {
mysql_query("UPDATE `api` SET `status`='3' WHERE api='$api'");
echo '{"error":-1,"msg":"<font color=blue><b>Api Key Expired</b></font> | '.$format.' | '.$cre.' [GATE:03] ./JH3 Ceker"}';
break;
}
if ($b == 5) {
echo '{"error":-1,"msg":"<font color=purple><b>Unknown</b></font> | '.$format.' | '.$cre.' [GATE:03] ./JH3 Ceker"}';
